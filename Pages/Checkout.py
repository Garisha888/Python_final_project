...
class Checkout:
    ...
    def checkout_product_list(self):
        ...
        length = len(checkoutproducts)
        print(length)
        # for x in checkoutproducts:
        checkoutproductsname = self.driver.find_elements(By.XPATH, "//div[@class='checkout-product-detail']//span")
        checkoutproductsquantity = self.driver.find_elements(By.XPATH, "//td[@class='text-center']")
        checkoutproductsprice = self.driver.find_elements(By.XPATH, "//tr[@data-item-added-from='ITEM_DETAILS']//td[4]")
